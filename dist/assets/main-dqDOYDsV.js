import{i as A,s as m,a as v,b as T,g as D,l as S,r as U,c as _,d as H}from"./navbar-CJXreWDP.js";/* empty css               */let p="all",y="all",L=[],k=[];const d=document.getElementById("uploadModal"),u=document.getElementById("resourceModal"),i=document.getElementById("authModal"),N=document.getElementById("postItemBtn"),G=document.getElementById("uploadResourceBtn"),P=document.getElementById("closeModal"),j=document.getElementById("closeResourceModal"),z=document.getElementById("closeAuthModal"),x=document.getElementById("uploadForm"),C=document.getElementById("resourceForm"),f=document.getElementById("loginForm"),h=document.getElementById("registerForm"),O=document.getElementById("searchInput"),X=document.getElementById("marketplaceGrid"),V=document.getElementById("resourcesGrid"),g=document.getElementById("loginBtn"),b=document.getElementById("logoutBtn"),E=document.getElementById("userMenu"),w=document.getElementById("dashboardLink");document.addEventListener("DOMContentLoaded",async()=>{await A(),await M(),await I(),await B(),Y()});async function I(){try{let e=m.from("marketplace_items").select("*").eq("status","active");p!=="all"&&(e=e.eq("category",p));const{data:t,error:o}=await e.order("created_at",{ascending:!1});if(o)throw o;L=t||[],$()}catch(e){console.error("Error fetching items:",e.message),r("Failed to load marketplace items")}}async function B(){try{let e=m.from("free_resources").select("*");y!=="all"&&(e=e.eq("type",y));const{data:t,error:o}=await e.order("created_at",{ascending:!1});if(o)throw o;k=t||[],F()}catch(e){console.error("Error fetching resources:",e.message),r("Failed to load resources")}}function Y(){N.addEventListener("click",()=>{if(!v()){l(i),r("Please login to post items");return}l(d)}),G.addEventListener("click",()=>{if(!v()){l(i),r("Please login to upload resources");return}l(u)}),P.addEventListener("click",()=>c(d)),j.addEventListener("click",()=>c(u)),z.addEventListener("click",()=>c(i)),d.addEventListener("click",o=>{o.target===d&&c(d)}),u.addEventListener("click",o=>{o.target===u&&c(u)}),i.addEventListener("click",o=>{o.target===i&&c(i)}),document.addEventListener("open-auth-modal",()=>{l(i)}),document.querySelectorAll(".auth-tab").forEach(o=>{o.addEventListener("click",n=>{const s=n.target.dataset.tab;document.querySelectorAll(".auth-tab").forEach(a=>a.classList.remove("active")),n.target.classList.add("active"),s==="login"?(f.style.display="block",h.style.display="none",document.getElementById("authModalTitle").textContent="Login"):(f.style.display="none",h.style.display="block",document.getElementById("authModalTitle").textContent="Register")})}),x.addEventListener("submit",J),C.addEventListener("submit",K),f.addEventListener("submit",se),h.addEventListener("submit",ae),g&&g.addEventListener("click",()=>l(i)),b&&b.addEventListener("click",q),document.querySelectorAll(".category-chip").forEach(o=>{o.addEventListener("click",n=>{document.querySelectorAll(".category-chip").forEach(s=>s.classList.remove("active")),n.target.classList.add("active"),p=n.target.dataset.category,I()})}),document.querySelectorAll(".resource-chip").forEach(o=>{o.addEventListener("click",n=>{document.querySelectorAll(".resource-chip").forEach(s=>s.classList.remove("active")),n.target.classList.add("active"),y=n.target.dataset.type,B()})}),O.addEventListener("input",te(ee,300));const e=document.querySelector(".file-upload-area"),t=document.getElementById("resourceFile");e&&t&&(e.addEventListener("click",()=>t.click()),t.addEventListener("change",o=>{var s;const n=(s=o.target.files[0])==null?void 0:s.name;n&&(e.querySelector("p").textContent=`Selected: ${n}`)}))}function l(e){e.classList.add("active"),document.body.style.overflow="hidden"}function c(e){e.classList.remove("active"),document.body.style.overflow=""}async function J(e){e.preventDefault();const t={title:document.getElementById("itemTitle").value,category:document.getElementById("itemCategory").value,price:parseFloat(document.getElementById("itemPrice").value),description:document.getElementById("itemDescription").value,condition:document.getElementById("itemCondition").value},o=await T(t);o.success?(I(),c(d),x.reset(),r("Item posted successfully!")):r(o.message)}async function K(e){e.preventDefault();const t={title:document.getElementById("resourceTitle").value,type:document.getElementById("resourceType").value,course:document.getElementById("resourceCourse").value,year:parseInt(document.getElementById("resourceYear").value)||new Date().getFullYear(),description:document.getElementById("resourceDescription").value};try{const{data:{user:o}}=await m.auth.getUser(),{error:n}=await m.from("free_resources").insert([{...t,uploader_id:o.id}]);if(n)throw n;B(),c(u),C.reset(),r("Resource uploaded successfully!")}catch(o){r(o.message)}}function $(){const e=L.filter(t=>p==="all"?!0:t.category===p);X.innerHTML=e.map(t=>Q(t)).join("")}function Q(e){return`
    ...
    `}function Z(e){return{new:"New","like-new":"Like New",good:"Good",fair:"Fair"}[e]||e}async function ee(e){const t=e.target.value.toLowerCase();try{let{data:o,error:n}=await m.from("marketplace_items").select("*").or(`title.ilike.%${t}%,description.ilike.%${t}%,category.ilike.%${t}%`).eq("status","active");if(n)throw n;L=o||[],$();let{data:s,error:a}=await m.from("free_resources").select("*").or(`title.ilike.%${t}%,course.ilike.%${t}%,type.ilike.%${t}%`);if(a)throw a;k=s||[],F()}catch(o){console.error("Search error:",o.message)}}function te(e,t){let o;return function(...s){const a=()=>{clearTimeout(o),e(...s)};clearTimeout(o),o=setTimeout(a,t)}}async function oe(e){if(!await v()){l(i),r("Please login to download resources");return}const t=k.find(o=>o.id===e);t&&(await H(t),r(`Downloading: ${t.title}`),console.log("Downloading resource:",t))}window.downloadResourceAction=oe;function r(e){const t=document.createElement("div");t.style.cssText=`
        position: fixed;
        top: 20px;
        right: 20px;
        background: #368CBF;
