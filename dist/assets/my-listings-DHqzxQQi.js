import{g as d,s as r}from"./auth-JaHXRiaM.js";/* empty css              */import"./admin-BcUQKC8m.js";document.addEventListener("DOMContentLoaded",async()=>{const t=await d();if(!t){window.location.href="index.html";return}await c(t.id)});async function c(t){const a=document.getElementById("myListingsGrid");try{const{data:e,error:o}=await r.from("market_listings").select("*").eq("seller_id",t).order("created_at",{ascending:!1});if(o)throw o;if(!e||e.length===0){a.innerHTML=`
            ...
            `;return}a.innerHTML=e.map(s=>g(s)).join(""),document.querySelectorAll(".status-select").forEach(s=>{s.addEventListener("change",async i=>{const n=i.target.dataset.id,l=i.target.value;await u(n,l)})}),document.querySelectorAll(".delete-btn").forEach(s=>{s.addEventListener("click",async i=>{const n=i.target.dataset.id;confirm("Are you sure you want to delete this listing? It cannot be undone.")&&await p(n)})})}catch(e){console.error("Error loading my listings:",e),a.innerHTML='<div style="color: red; padding: 2rem;">Error loading listings. Please refresh.</div>'}}function g(t){let a="https://via.placeholder.com/300x200?text=No+Image";if(t.images&&Array.isArray(t.images)&&t.images.length>0)a=t.images[0];else if(typeof t.images=="string"&&t.images.startsWith("["))try{const e=JSON.parse(t.images);e.length>0&&(a=e[0])}catch{}return`
